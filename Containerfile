...
RUN --mount=type=tmpfs,dst=/tmp --mount=type=tmpfs,dst=/root --mount=type=tmpfs,dst=/boot apt update -y && \
    apt-get update -y && apt-get -y install wget curl gpg lsb-release && wget -qO - https://dl.xanmod.org/archive.key | gpg --dearmor -vo /etc/apt/keyrings/xanmod-archive-keyring.gpg && \
    ...
    apt update -y && apt install -y linux-xanmod-lts-x64v3 scx-scheds clang libelf-dev lld llvm && \
    apt install -y btrfs-progs dosfstools e2fsprogs fdisk firmware-linux-free skopeo systemd systemd-boot* xfsprogs && \
...
#
    ...
    apt remove -y gnome-software packagekit firefox-esr showtime gnome-maps snapshot simple-scan gnome-connections gnome-contacts gnome-calculator gnome-clocks gnome-weather \
    && apt-get clean \
